...
function kasutan_single_entry_content_before() {
	...
	echo '<div class="meta-single">';
		printf('<p class="date">%s <strong>%s</strong>',__('Publié le :','croissancebleue'),get_the_date('d/m/Y'));
		printf('<p class="author">%s <strong>%s</strong>',__('Par :','croissancebleue'),$auteurs);
		printf('<p class="cats">%s %s',__('Catégorie thématique :','croissancebleue'),$list);
	echo '</div>';
