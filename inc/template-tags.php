...
function ea_post_summary_image( $size = 'thumbnail' ) {
	...
	if(has_post_thumbnail()) {
		...
		$banniere=get_field('cb_page_banniere');
		if($banniere) {
			$image_id=$banniere;
			$classe='';
		} else {
			$logo_footer=get_field('cb_logo_footer','option');
			if($logo_footer) {
...
function kasutan_affiche_liste_cats() {
	...
	echo '<nav class="nav-cat">';
		printf('<p class="titre-nav h3">%s</p>',__('Accès thématiques','croissancebleue'));
...
function kasutan_boutons_partage() {
	...
	$labels=array();
	$labels['facebook']=__('Partager sur Facebook','croissancebleue');
	$labels['twitter']=__('Partager sur Twitter','croissancebleue');
	$labels['linkedin']=__('Partager sur LinkedIn','croissancebleue');
	$labels['mail']=__('Partager par email','croissancebleue');

	$titre=false;
	if(function_exists('get_field')) {
		$titre=wp_kses_post(get_field('cb_titre_partage','option'));
	}
	if(!$titre) {
		$titre=__('Partager cet article','croissancebleue');
	}
...
function kasutan_affiche_trois_articles($term, $titre_section='', $exclude=array(),$className='',$label_bouton='') {
	...
	if(!$titre_section && $term && function_exists('get_field')) {
		$titre_section=sprintf('%s <span class="term">%s</span>',
			wp_kses_post(get_field('cb_titre_related','option')),
			$term->name
		);
	}
