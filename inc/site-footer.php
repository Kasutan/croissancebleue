...
function kasutan_main_footer() {
	...
	if(function_exists('get_field')) {
		...
		$coord=wp_kses_post(get_field('cb_footer_coordonnees','option'));
		$titres[2]=wp_kses_post(get_field('cb_footer_titre_2','option'));
		$titres[3]=wp_kses_post(get_field('cb_footer_titre_3','option'));
		$titres[4]=wp_kses_post(get_field('cb_footer_titre_4','option'));
	}
...
function kasutan_copyright() {
	...
	echo '<div class="bottom-footer">';
		printf('<span class="copyright">%s %s</span>',esc_html__('&copy; Tous droits réservés','croissancebleue'),get_option('blogname'));
